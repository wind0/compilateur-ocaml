...
let rec print_sign = fun s chan father max ->
...
let rec print_constant_list = print_lister "CONST_LIST" print_constant

let rec print_simple_type = fun st chan father max ->
match st with
...
let rec print_simple_type_list = print_lister "SIMPLE_TYPE_LIST" print_simple_type


(*Type automate*)


let rec print_field_list = let rec truc = fun fl chan father max ->
match fl with
...
let rec print_log_operator = fun op chan father max ->
let my_print = fun a -> print_init a chan father max
in
match op with
|Lt -> my_print "<"
...
(* termes *)
let rec print_op_term = fun op chan father max ->
let my_print = fun a -> print_init a chan father max
in
match op with
|Times -> my_print "*"
...
let rec print_init_type = fun (id, typ_auto) chan father max ->
let position = print_init "INIT_TYPE" chan father max
in let position1 = print_id id chan position position
in print_typ_auto typ_auto chan position position1	

let rec print_block_type = print_lister "BLOCK_TYPE" print_init_type	


let rec print_init_const = fun (identifier, constante) chan father max ->
...
let rec print_block_const = print_lister "init_cont list" print_init_const 
...
let rec print_init_var = fun (id_list,typ_auto) chan father max ->
let position = print_init "INIT_VAR" chan father max
in let position1 = print_id_list id_list chan position position
in print_typ_auto typ_auto chan position position1

let rec print_block_var = print_lister "INIT_VAR LIST" print_init_var
...
let rec print_expr_or_procid = fun eop chan father max ->
match eop with
|Id id -> print_id id chan father max
|Expr exp -> print_expr exp chan father max

let rec print_expr_id_list = print_lister "EXP_OR_PROCID_LIST" print_expr_or_procid



let rec print_var_or_id = fun a chan father max ->
match a with
|Variable var -> print_var var chan father max
|Id2 id -> print_id id chan father max

let rec print_inc_or_decr = fun a chan father max ->
match a with
...
let rec print_statement_list = print_lister "Statement list" print_statement
